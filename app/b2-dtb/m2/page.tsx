...
import { m2Redemittel } from "../../../lib/redemittel";
...
export default function M2Page() {
  ...
  const trainingSituation = m2Situations[level - 1];
  const simulationSituation =
    randomIndex !== null ? m2Situations[randomIndex] : null;
  ...
  useEffect(() => {
    ...
    const random = Math.floor(Math.random() * m2Situations.length);
  ...
  const structureHints = useMemo(() => {
    ...
    return (
      <>
        ...
        <h5>Einleitung</h5>
        <ul>
          {m2Redemittel.structure.introduction.map((r: string, i: number) => (
            <li key={i}>{r}</li>
          ))}
        </ul>

        {/* Meinung v. a. bei schwachem Ausdruck */}
        {result.expression < 12 && (
          <>
            <h5>Eigene Meinung</h5>
            <ul>
              {m2Redemittel.structure.opinion.map((r: string, i: number) => (
                <li key={i}>{r}</li>
              ))}
            </ul>
          </>
        ...
        {/* Vorteil/Nachteil v. a. bei schwacher Kohäsion */}
        {result.cohesion < 12 && (
          <>
            <h5>Vorteil / Nachteil</h5>
            <ul>
              {m2Redemittel.structure.advantage.map((r: string, i: number) => (
                <li key={`a-${i}`}>{r}</li>
              ))}
              {m2Redemittel.structure.disadvantage.map((r: string, i: number) => (
                <li key={`d-${i}`}>{r}</li>
              ))}
            </ul>
          </>
        ...
        {/* Vorschlag nur, wenn nicht erkannt */}
        {!result.functions?.solution && (
          <>
            <h5>Vorschlag</h5>
            <ul>
              {m2Redemittel.structure.proposal.map((r: string, i: number) => (
                <li key={i}>{r}</li>
              ))}
            </ul>
          </>
        ...
        <h5>Schluss</h5>
        <ul>
          {m2Redemittel.structure.conclusion.map((r: string, i: number) => (
            <li key={i}>{r}</li>
          ))}
        </ul>
      </>
  ...
  return (
    <main style={{ padding: 40, maxWidth: 1100, margin: "auto" }}>
      ...
      <div style={taskBox}>
        <h3>Aufgabe</h3>
        <pre style={{ whiteSpace: "pre-wrap" }}>{situation.task}</pre>
      </div>
      ...
      {result && (
        <div style={resultBox}>
          ...
          <ul>
            <li>Einleitung/Problem: {result.functions?.problem ? "✔" : "✘"}</li>
            <li>Vorschlag/Lösung: {result.functions?.solution ? "✔" : "✘"}</li>
          </ul>
